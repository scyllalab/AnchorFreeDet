...
from sample.voc import deal_batch
from test.voc_eval import voc_eval
import cv2
import numpy as np
...
class AnnotationTransform(object):
    ...
    def __init__(self, class_to_ind=None, keep_difficult=False):
        self.class_to_ind = class_to_ind or dict(
            zip(NOMOTOR_CLASSES, range(len(NOMOTOR_CLASSES))))
...
class NOMOTORDetection(data.Dataset):
    ...
    def __init__(self,root,image_sets,dataset_name='NOMOTOR'):
        self.root = root
    ...
    def _get_voc_results_file_template(self):
        ...
        filedir = os.path.join(self.root, 'results', 'NOMOTOR' + self._year,
                               'Main')
    ...
    def _write_voc_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(NOMOTOR_CLASSES):
            print('Writing {} NOMOTOR results file'.format(cls))
    ...
    def _do_python_eval(self, output_dir='output'):
        ...
        if output_dir is not None and not os.path.isdir(output_dir):
            os.mkdir(output_dir)
        for i, cls in enumerate(NOMOTOR_CLASSES):
            filename = self._get_voc_results_file_template().format(cls)
